...
export class EventService {
  ...
  protected convertDateFromClient(event: IEvent): IEvent {
    return Object.assign({}, event, {
      startAt: event.startAt?.isValid() ? event.startAt.toJSON() : undefined,
      finishedAt: event.finishedAt?.isValid() ? event.finishedAt.toJSON() : undefined,
    });
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.finishedAt = res.body.finishedAt ? dayjs(res.body.finishedAt) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((event: IEvent) => {
        ...
        event.finishedAt = event.finishedAt ? dayjs(event.finishedAt) : undefined;
      });
    }
    return res;
  }
